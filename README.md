...
技術スタックは Python, Streamlit, SQLite, pandas, matplotlib, python-docx, PyYAML, pytest です。現時点ではサンプルCSVで財務分析が動作し、EDINET APIは書類一覧取得とCSV ZIP取得まで対応しています。完全なXBRL/CSV解析と財務データ自動反映は今後の拡張対象です。
...
- 取得したCSV ZIPから主要財務タグ候補を抽出し、分析用データ候補を確認する
...
現時点の財務指標計算はサンプルCSVを使います。取得済みEDINET CSV ZIPから主要財務タグ候補を抽出する入口はありますが、取得値をレポート計算へ自動反映する処理は、XBRL/CSV解析強化の拡張対象です。
